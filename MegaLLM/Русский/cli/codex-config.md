...
> Настройка Codex и Windsurf для использования MegaLLM
...
1. **Единственный экземпляр** - Codex/Windsurf запускается как единственный экземпляр для всех проектов
2. **Глобальные настройки** - Настройки инструментов применяются системно
...
<CardGroup cols={2}>
  <Card title="Резервная конфигурация" icon="floppy-disk">
  ...
  <Card title="Контроль версий" icon="code-branch">
    Вы можете коммитить `config.toml`, если используется env\_key (без жестко закодированных ключей)
  </Card>

  <Card title="Регулярные обновления" icon="rotate">
    Поддерживайте Codex/Windsurf обновленными для последних функций
  </Card>
</CardGroup>
