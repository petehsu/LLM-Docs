...
* English
 ...
 [API Login](/login?redirect=%2Fdev%2Fapi%2Fthirdparty-frame%2Flangchain-sdk)
...
# Installing Langchain SDK

First, you need to install Langchain and its corresponding dependencies. Ensure that the version of `langchain_community` is above 0.0.32.
...
The `ChatOpenAI` class in Langchain is a wrapper for the `OpenAI` SDK, making it easier to call. This demonstrates how to use the `ChatOpenAI` class to call the `GLM-4` model.
...
## Using LangChain AgentExecutor

Additionally, `GLM-4` can be seamlessly integrated into Langchain’s `AgentExecutor`. This demonstrates how to use `AgentExecutor` to call the `GLM-4` model.
