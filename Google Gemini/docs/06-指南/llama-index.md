...
During execution of the workflow, you can stream events, tool calls and updates to the console.  
    ...
    async for event in handler.stream_events():
        if (
            ...
            print(f"\n{'='*50}")
            print(f"ð¤ Agent: {current_agent}")
            print(f"{'='*50}\n")
        elif isinstance(event, AgentOutput):
            if event.response.content:
                print("ð¤ Output:", event.response.content)
            if event.tool_calls:
                print(
                    "ð ï¸  Planning to use tools:",
                    [call.tool_name for call in event.tool_calls],
                )
        elif isinstance(event, ToolCallResult):
            print(f"ð§ Tool Result ({event.tool_name}):")
            print(f"  Arguments: {event.tool_kwargs}")
            print(f"  Output: {event.tool_output}")
        elif isinstance(event, ToolCall):
            print(f"ð¨ Calling Tool: {event.tool_name}")
