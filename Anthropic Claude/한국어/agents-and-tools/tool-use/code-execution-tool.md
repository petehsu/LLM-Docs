...
코드 실행 도구를 사용하면 Claude가 보안 샌드박스 환경에서 Bash 명령을 실행하고 파일을 조작할 수 있으며, 코드 작성도 포함됩니다.
...
1. **파일 업로드** [Files API](/docs/ko/build-with-claude/files)를 사용하여
2. **파일 참조** `container_upload` 콘텐츠 블록을 사용하여 메시지에서
3. **코드 실행 도구 포함** API 요청에서
...
| text_editor | `file_not_found` | 파일이 존재하지 않습니다 (보기/편집 작업의 경우) |
| text_editor | `string_not_found` | `old_str`을 파일에서 찾을 수 없습니다 (str_replace의 경우) |

#### `pause_turn` 중지 이유

응답에는 `pause_turn` 중지 이유가 포함될 수 있으며, 이는 API가 오래 실행되는 턴을 일시 중지했음을 나타냅니다. 응답을 그대로 후속 요청에 제공하여 Claude가 턴을 계속하도록 하거나, 대화를 중단하려면 콘텐츠를 수정할 수 있습니다.
...
- **작업 공간 범위**: [Files](/docs/ko/build-with-claude/files)와 마찬가지로 컨테이너는 API 키의 작업 공간으로 범위가 지정됩니다
...
[Messages Batches API](/docs/ko/build-with-claude/batch-processing)에 코드 실행 도구를 포함할 수 있습니다. Messages Batches API를 통한 코드 실행 도구 호출은 일반 Messages API 요청과 동일한 가격으로 책정됩니다.
...
`code-execution-2025-08-25`로 업그레이드하면 파일 조작 및 Bash 기능을 포함한 여러 언어의 코드에 액세스할 수 있습니다. 가격 차이는 없습니다.
...
[프로그래밍 방식 도구 호출 문서](/docs/ko/agents-and-tools/tool-use/programmatic-tool-calling)에서 자세히 알아보세요.
...
[Agent Skills 문서](/docs/ko/agents-and-tools/agent-skills/overview) 및 [Agent Skills API 가이드](/docs/ko/build-with-claude/skills-guide)에서 자세히 알아보세요.