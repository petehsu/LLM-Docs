...
* English
 ...
 [API Login](/login?redirect=%2Fdev%2Fapi%2Ferror-code%2Ferror-code-v4)
...
When calling the API, the received response code consists of two parts: the outer layer is the HTTP status code, and the inner layer is the business error code defined by Zhipu AI in the response body, which provides more specific error descriptions.
...
| 429 | Account anomaly | Account has违规行为, please contact platform customer service or service@zhipuai.cn to unlock |
| 429 | Terminal account anomaly | Terminal user has违规行为, account has been locked |
...
Below is the response message from a curl request, where 401 is the HTTP status code and 1002 is the business error code.
...
{"error":{"code":"1002","message":"Invalid Authorization Token, please confirm the correct Authorization Token is passed."}}
```
