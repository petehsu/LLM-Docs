...
返回一个 [`Query`](#query-1) 对象，该对象扩展 `AsyncGenerator<`[`SDKMessage`](#sdkmessage)`, void>` 并具有额外的方法。
...
| `canUseTool` | [`CanUseTool`](#canusetool) | `undefined` | 用于控制工具使用的自定义权限函数 |
...
| `outputFormat` | `{ type: 'json_schema', schema: JSONSchema }` | `undefined` | 为代理结果定义输出格式。有关详细信息，请参阅[结构化输出](/docs/zh-CN/agent-sdk/structured-outputs) |
...
| `plugins` | [`SdkPluginConfig`](#sdkpluginconfig)`[]` | `[]` | 从本地路径加载自定义插件。有关详细信息，请参阅[插件](/docs/zh-CN/agent-sdk/plugins) |
| `resume` | `string` | `undefined` | 要恢复的会话 ID |
| `resumeSessionAt` | `string` | `undefined` | 在特定消息 UUID 处恢复会话 |
| `sandbox` | [`SandboxSettings`](#sandboxsettings) | `undefined` | 以编程方式配置沙箱行为。有关详细信息，请参阅[沙箱设置](#sandboxsettings) |
| `settingSources` | [`SettingSource`](#settingsource)`[]` | `[]`（无设置） | 控制要加载的文件系统设置。省略时，不加载任何设置。**注意：** 必须包含 `'project'` 以加载 CLAUDE.md 文件 |
...
| `tools` | `string[] \| { type: 'preset'; preset: 'claude_code' }` | `undefined` | 工具配置。传递工具名称数组或使用预设以获取 Claude Code 的默认工具 |
...
有关使用钩子的综合指南，包括示例和常见模式，请参阅[钩子指南](/docs/zh-CN/agent-sdk/hooks)。
...
**注意：** 这是仅用于清晰起见的文档类型。它表示所有工具输入类型的并集。
...
在执行期间向用户询问澄清问题。有关使用详情，请参阅[处理 AskUserQuestion 工具](/docs/zh-CN/agent-sdk/permissions#handling-the-askuserquestion-tool)。
...
**注意：** 这是仅用于清晰起见的文档类型。它表示所有工具输出类型的并集。
...
| `excludedCommands` | `string[]` | `[]` | 始终绕过沙箱限制的命令（例如 `['docker']`）。这些命令自动运行在沙箱外，无需模型参与 |
...
<Note>
**文件系统和网络访问限制**不是通过沙箱设置配置的。相反，它们来自[权限规则](https://code.claude.com/docs/zh-CN/settings#permission-settings)：
...
启用 `allowUnsandboxedCommands` 时，模型可以通过在工具输入中设置 `dangerouslyDisableSandbox: true` 来请求在沙箱外运行命令。这些请求会回退到现有权限系统，这意味着你的 `canUseTool` 处理程序将被调用，允许你实现自定义授权逻辑。

<Note>
**`excludedCommands` vs `allowUnsandboxedCommands`：**
- `excludedCommands`：始终自动绕过沙箱的静态命令列表（例如 `['docker']`）。模型对此无法控制。
