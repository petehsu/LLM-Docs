...
为每次与 Claude Code 的交互创建一个新会话。返回一个异步迭代器，当消息到达时产生消息。每次调用 `query()` 都会从头开始，不记得之前的交互。
...
> **重要：** 在迭代消息时，避免使用 `break` 提前退出，因为这可能导致 asyncio 清理问题。相反，让迭代自然完成或使用标志来跟踪何时找到了你需要的内容。
...
| `output_format`               | [`OutputFormat`](#outputformat) ` \| None`   | `None`               | 为代理结果定义输出格式。详见 [结构化输出](/docs/zh-CN/agent-sdk/structured-outputs)                                                                    |
...
| `include_partial_messages`    | `bool`                                       | `False`              | 包括部分消息流事件                                                                                                                                                                |
...
| `setting_sources`             | `list[SettingSource] \| None`                | `None`（无设置） | 控制要加载哪些文件系统设置。省略时，不加载任何设置。**注意：** 必须包含 `"project"` 以加载 CLAUDE.md 文件                                             |
...
使用 Claude Code 的预设系统提示（可选添加）的配置。
...
有关使用 hook 的综合指南、示例和常见模式，请参阅 [Hooks 指南](/docs/zh-CN/agent-sdk/hooks)。
...
- `input_data`: Hook 特定的输入数据（参见 [Hooks 指南](/docs/zh-CN/agent-sdk/hooks#input-data)）
- `tool_use_id`: 可选的工具使用标识符（用于工具相关的 hook）
- `context`: Hook 上下文，包含其他信息
...
class HookMatcher:
    ...
    timeout: float | None = None        # 此匹配器中所有 hook 的超时时间（秒）（默认值：60）
```

### Hook 使用示例

此示例注册了两个 hook：一个阻止危险的 bash 命令（如 `rm -rf /`），另一个记录所有工具使用情况以进行审计。安全 hook 仅在 Bash 命令上运行（通过 `matcher`），而日志 hook 在所有工具上运行。
...
```python
{
    ...
    "filter": str | None  # 可选的正则表达式以筛选输出行
}
```
...
```python
{
    "server": str | None  # 可选的服务器名称以按其筛选资源
}
```
...
沙箱行为的配置。使用此选项以编程方式启用命令沙箱化并配置网络限制。
...
<Note>
**文件系统和网络访问限制**不通过沙箱设置配置。相反，它们来自[权限规则](https://code.claude.com/docs/en/settings#permission-settings)：
...
当启用 `allowUnsandboxedCommands` 时，模型可以通过在工具输入中设置 `dangerouslyDisableSandbox: True` 来请求在沙箱外运行命令。这些请求回退到现有权限系统，这意味着您的 `can_use_tool` 处理程序将被调用，允许您实现自定义授权逻辑。
