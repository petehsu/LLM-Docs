...
<Note>
**새로운 V2 인터페이스 시도 (미리보기):** `send()` 및 `receive()` 패턴이 있는 단순화된 인터페이스가 이제 사용 가능하며, 다중 턴 대화를 더 쉽게 만듭니다. [자세히 알아보기](/docs/ko/agent-sdk/typescript-v2-preview)
</Note>
...
| `options.tools` | `Array<SdkMcpToolDefinition>` | [`tool()`](#tool)로 만든 도구 정의 배열 |
...
| `forkSession` | `boolean` | `false` | `resume`으로 재개할 때 원본 세션을 계속하는 대신 새 세션 ID로 포크 |
...
| `settingSources` | [`SettingSource`](#settingsource)`[]` | `[]` (설정 없음) | 로드할 파일시스템 설정을 제어합니다. 생략하면 설정이 로드되지 않습니다. **참고:** CLAUDE.md 파일을 로드하려면 `'project'`를 포함해야 합니다 |
| `stderr` | `(data: string) => void` | `undefined` | stderr 출력에 대한 콜백 |
| `strictMcpConfig` | `boolean` | `false` | 엄격한 MCP 검증 적용 |
| `systemPrompt` | `string \| { type: 'preset'; preset: 'claude_code'; append?: string }` | `undefined` (빈 프롬프트) | 시스템 프롬프트 구성. 사용자 정의 프롬프트의 경우 문자열을 전달하거나, Claude Code의 시스템 프롬프트를 사용하려면 `{ type: 'preset', preset: 'claude_code' }`를 전달합니다. 프리셋 객체 형식을 사용할 때 `append`를 추가하여 추가 지침으로 시스템 프롬프트를 확장합니다 |
| `tools` | `string[] \| { type: 'preset'; preset: 'claude_code' }` | `undefined` | 도구 구성. 도구 이름 배열을 전달하거나 프리셋을 사용하여 Claude Code의 기본 도구를 가져옵니다 |
...
`settingSources`가 **생략**되거나 **undefined**인 경우, SDK는 파일시스템 설정을 로드하지 **않습니다**. 이는 SDK 애플리케이션에 격리를 제공합니다.
...
// 파일시스템 종속성 없음 - settingSources는 기본값이 []
const result = query({
  prompt: "이 PR을 검토하세요",
  options: {
...
const result = query({
  ...
  options: {
    systemPrompt: {
      type: 'preset',
      preset: 'claude_code'  // CLAUDE.md 사용 필수
    },
...
interface AgentInput {
  ...
  prompt: string;
  /**
   * 이 작업에 사용할 특화된 에이전트의 타입
   */
  subagent_type: string;
}
```

복잡한 다단계 작업을 자동으로 처리할 새로운 에이전트를 시작합니다.
...
선택적 타임아웃 및 백그라운드 실행이 있는 지속적인 셸 세션에서 bash 명령을 실행합니다.
...
interface FileEditInput {
  ...
  new_string: string;
  /**
   * old_string의 모든 발생을 바꾸기 (기본값 false)
   */
  replace_all?: boolean;
}
```
...
텍스트, 이미지, PDF 및 Jupyter 노트북을 포함한 로컬 파일시스템에서 파일을 읽습니다.
...
ripgrep을 기반으로 한 정규식 지원이 있는 강력한 검색 도구입니다.
...
interface TaskOutput {
  /**
   * 위임된 작업을 완료한 후 서브에이전트의 최종 결과 메시지
   */
  result: string;
  /**
   * 토큰 사용 통계
...
종료 상태가 있는 명령 출력을 반환합니다. 백그라운드 명령은 shellId와 함께 즉시 반환됩니다.
...
glob 패턴과 일치하는 파일 경로를 수정 시간순으로 정렬하여 반환합니다.
...
샌드박스 동작에 대한 구성입니다. 이를 사용하여 명령 샌드박싱을 활성화하고 프로그래매틱 방식으로 네트워크 제한을 구성합니다.
...
| `network` | [`NetworkSandboxSettings`](#networksandboxsettings) | `undefined` | 네트워크 특정 샌드박스 구성 |
| `ignoreViolations` | [`SandboxIgnoreViolations`](#sandboxignoreviolations) | `undefined` | 무시할 샌드박스 위반을 구성합니다 |
...
| 속성 | 타입 | 기본값 | 설명 |
| :------- | :--- | :------ | :---------- |
| `allowLocalBinding` | `boolean` | `false` | 프로세스가 로컬 포트에 바인딩하도록 허용 (예: 개발 서버의 경우) |
...
- `excludedCommands`: 항상 자동으로 샌드박스를 우회하는 명령의 정적 목록 (예: `['docker']`). 모델은 이에 대한 제어권이 없습니다.
