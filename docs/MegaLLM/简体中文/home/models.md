...
> 通过单一统一 API 访问领先提供商的前沿 AI 模型。所有模型都可以在 API 调用中使用其模型 ID 访问。
...
<Note>
  模型目录会动态更新。访问我们的[控制台](https://megallm.io/dashboard/models)获取实时定价和可用性信息。
</Note>
...
<CardGroup cols={2}>
  <Card title="快速响应" icon="bolt-lightning">
    gpt-5-mini, gpt-4o-mini, gemini-2.0-flash-001, gpt-3.5-turbo
  </Card>

  <Card title="复杂推理" icon="brain-circuit">
    gpt-5, claude-opus-4-1-20250805, gemini-2.5-pro
  </Card>

  <Card title="高性价比" icon="coins">
    gpt-4o-mini, gemini-2.0-flash-001
  </Card>

  <Card title="大上下文" icon="file-contract">
    gpt-4.1 (1M+), gemini-2.5-pro (1M+)
  </Card>

  <Card title="视觉任务" icon="images">
    gpt-5, gpt-4o, claude-sonnet-4, gemini 模型
  </Card>

  <Card title="代码生成" icon="brackets-curly">
...
<CardGroup cols={2}>
  <Card title="经济型" icon="wallet">
    **模型**: `gpt-4o-mini`, `gemini-2.0-flash-001`
  ...
  <Card title="标准型" icon="gauge-high">
    **模型**: `gpt-5-mini`, `claude-3.5-sonnet`
  ...
  <Card title="高级型" icon="crown">
    **模型**: `gpt-5`, `claude-sonnet-4`
  ...
  <Card title="企业型" icon="building-columns">
    **模型**: `claude-opus-4-1-20250805`, `gpt-4.1`
...
<CodeGroup>
  ...
  # 使用模型 ID 切换到 Claude
  response = client.chat.completions.create(
      model="claude-opus-4-1-20250805",  # 模型 ID
      messages=[{"role": "user", "content": "你好！"}]
  )

  # 使用模型 ID 尝试 Gemini
  response = client.chat.completions.create(
      model="gemini-2.5-pro",  # 模型 ID
      messages=[{"role": "user", "content": "你好！"}]
  )
  ```
  ...
  for (const modelId of models) {
    const response = await fetch("https://ai.megallm.io/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: modelId,  // 使用模型 ID
        messages: [{ role: "user", content: "你好！" }]
      })
    });

    console.log(`${modelId} 响应:`, await response.json());
  }
  ```
...
| 使用级别     | Token/月 | gpt-5-mini | claude-3.5-sonnet | gemini-2.0-flash-001 |
| -------- | ------- | ---------- | ----------------- | -------------------- |
| **爱好者**  | 1M      | \$2.25     | \$18              | \$0.75               |
| **初创企业** | 10M     | \$22.50    | \$180             | \$7.50               |
| **企业**   | 100M    | \$225      | \$1,800           | \$75                 |
| **大型企业** | 1B+     | 定制         | 定制                | 定制                   |

<Warning>
  **重要**: 模型 ID 区分大小写。始终使用上表中显示的确切模型 ID。
</Warning>

## 下一步

* 阅读[常见问题](/cn/home/faq)了解有关模型选择的常见问题
* 查看[API 参考](/cn/api-reference/introduction)获取详细的端点文档
* 查看[开发者文档](/cn/dev-docs/overview)获取集成指南
