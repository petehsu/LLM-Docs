...
* English
 ...
 [API Login](/login?redirect=%2Fdev%2Fapi%2Fparameter-description)
...
0 new users invited this month.

Received 0 ten thousand for GLM-4.5-Air.  View

You haven't started sharing this month. Share your invite link to earn 20 million GLM-4.5-Air Tokens when the invitee verifies!
...
| --- | --- |
| api\_key | User request authentication information, which the user needs to store by themselves. This information should be avoided from being transmitted on the public network to prevent unnecessary losses. |
| model | The model code to be called. |
| messages | When calling a dialogue model, the current dialogue information list is input as a prompt to the model in the form of key-value pairs like {“role”: “user”, “content”: “Hello”}. - role is required, the role of the message author, which can be either user or assistant. - content is required, the specific content of the message. Examples: Example of a single dialogue message `{"messages": [{"role": "user", "content": "Hello"}] }` Example of multiple dialogue histories `{ "messages": [ {"role": "user", "content": "Hello"}, {"role": "assistant", "content": "Hello! I am the AI assistant ChatGLM, very pleased to meet you, feel free to ask me any questions."}, {"role": "user", "content": "What can you do?"} ]}` |
| temperature | Sampling temperature, which controls the randomness of the output, must be a positive number. The value range is: `(0.0,1.0]`, cannot be equal to 0. The larger the value, the more random and creative the output will be; the smaller the value, the more stable or deterministic the output will be. It is recommended that you adjust the `top_p` or `temperature` parameters according to the application scenario, but do not adjust both parameters at the same time. |
| top\_p | Another method of temperature sampling, called nucleus sampling. The value range is: `(0.0,1.0)`; an open interval, cannot be equal to 0 or 1. The model considers the results of tokens with top\_p probability mass. So 0.1 means the model decoder only considers tokens from the top 10% probability candidate set. It is recommended that you adjust the `top_p` or `temperature` parameters according to the application scenario, but do not adjust both parameters at the same time. |